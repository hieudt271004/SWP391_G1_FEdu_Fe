...
import { ArrowLeft, CheckCircle2, Circle, Upload, Map, Loader } from 'lucide-react';
...
export function ClassManagementPage() {
  ...
  useEffect(() => {
    const fetchClassroomData = async () => {
      ...
      try {
        ...
        console.log('Students fetched for class management:', studentsData);
  ...
  const getModuleIcon = (status: Module['status']) => {
    switch (status) {
      case 'completed':
  ...
  const handleAddContent = (moduleId: number) => {
    console.log('Adding content to module:', moduleId);
  };
  ...
  const handleManageRoadmap = () => {
    console.log('Managing roadmap');
  };

  const handleUpMaterials = () => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Button onClick={handleManageRoadmap}>
            <Map className="size-4" />
            Manage Roadmap
          </Button>
          <Button onClick={handleUpMaterials}>
              ...
              {modules.map((module, index) => (
                <div key={module.id} className="relative">
                  ...
                  <div className="flex items-start gap-4 relative">
                    <div
                      className={`flex items-center justify-center size-12 rounded-full border-2 shrink-0 ${getModuleColor(
                        module.status
                      )}`}
                    >
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Roadmap Completion</TableHead>
