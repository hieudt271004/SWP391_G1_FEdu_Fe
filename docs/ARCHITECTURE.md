...
├── services/
│   └── auth.service.ts      loginAPI, registerAPI, getMeAPI, ...
└── types/
...
→ getMeAPI(accessToken)                        GET /auth/me
→ setUser(userData)
→ navigate("/student/dashboard")                  hardcoded tạm, sẽ smart redirect sau
...
- Service function: camelCase với suffix `API` (`loginAPI`, `getMeAPI`)
